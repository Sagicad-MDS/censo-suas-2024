...
A partir das informações contidas no Censo SUAS é possível ter um panorama geral da gestão e do financiamento em estados e municípios. Este panorama pode ser obtido pela observação de aspectos da estrutura administrativa da gestão da Assistência Social, da institucionalização da política através de lei, do apoio de estados aos municípios, das ações de cofinanciamento e transferência de recursos, do funcionamento das instâncias de pactuação, entre outros.

A estrutura administrativa na Política de Assistência Social e a constituição de setores essenciais são elementos importantes para a gestão do SUAS. No âmbito da gestão estadual percebe-se que desde 2012 há uma redução de secretarias estaduais exclusivas de Assistência Social. Em 2012, 8 estados (Acre, Amazonas, Pará, Amapá, Piauí, Paraíba, Sergipe e São Paulo) tinham secretarias estaduais exclusivas da área de assistência social. Esse número se reduziu até chegar em apenas **2 estados** em 2018, mesmo número observado em 2024, quando o Estado do **Amapá** se somou ao Estado do **Amazonas** após 3 anos em que apenas o Estado do Amazonas tinha **secretaria estadual exclusiva da área de assistência social**.

```{r fig-estados_sec_exc, fig.cap = "Percentual de estados quanto à característica de estrutura administrativa de Assistência Social - Brasil, 2012 a 2024", fig.footer = "Fonte: MDS, Censo SUAS"}
...
grafico <- estados_sec_exc(ge_2012, q1, "2012") %>%
  ...
  bind_rows(estados_sec_exc(ge_2024, q1, "2024")) %>%
...
```{r fig-estados-caracteristica-secretaria, fig.cap = "Quantidade de Secretarias Estaduais de acordo com composição administrativa em conjunto com outras políticas setoriais - Brasil, 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
...
Na estrutura administrativa da Assistência Social nos municípios observa-se em 2024 uma redução em relação a 2023, de 81,6% para **80,8%**, no percentual de municípios que possuem **secretaria municipal exclusiva da área de Assistência Social** (@fig-sec-munic-exc). Ainda assim é um percentual maior do que os percentuais registrados entre 2012 a 2019, período em que esse percentual oscilou entre 76,6% e 79,3% (de 2020 a 2022 essa pergunta foi retirada do formulário da Gestão Municipal).

```{r fig-sec-munic-exc, fig.cap = "Percentual de Secretarias Municipais quanto a caracteristica de estrutura administrativa - Brasil, 2012 a 2024", fig.footer = "Fonte: MDS, Censo SUAS"}
...
grafico <-
 ...
 # bind_rows(sec_munic_exc(gm_2022, q1, "2022")) %>% 
   ...
   bind_rows(sec_munic_exc(gm_2024, q1, "2024")) %>% 
...
Para o aprimoramento da gestão do SUAS entende-se ser necessário que as diversas áreas da Assistência Social sejam constituídas formalmente na estrutura do órgão gestor. Alguns órgãos gestores estaduais constituíram as áreas de assistência social como subdivisões administrativas em sua estrutura, como superintendências, departamentos, gerências, coordenações, dentre outras.

No @fig-uf_subd observamos que tem avançado a constituição formal das áreas da Assistência Social na estrutura administrativa dos órgãos gestores estaduais. Em 2024, a única área que não avançou em relação a 2023 foi a de Gestão do Cadastro Único e Programa Bolsa Família, que caiu de 21 para 20 estados com esta subdivisão administrativa constituída formalmente na estrutura do órgão gestor. Mesmo assim, manteve-se constituída em 100% dos estados, quando considerada também a constituição informal.

As áreas mais instituídas formalmente na estrutura do órgão gestor dos estados em 2024 são: Proteção Social Básica e Proteção Social Especial, Gestão Financeira e Orçamentária e Gestão do SUAS, todas tendo 92,3% dos estados (24 estados) com essas áreas constituídas formalmente. Destaca-se em 2024 o **avanço da constituição formal da área de Regulação do SUAS, subindo de 42,3% dos estados em 2023 para 69,2% em 2024**.

```{r fig-uf_subd, fig.cap = "Percentual de estados segundo constituição de subdivisões administrativas na estrutura do órgão gestor - Brasil; 2012, 2018, 2023 e 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
#| fig-height: 8
...
#  bind_rows(uf_subd(ge_2020,q3_4,"2020","9")) %>%
  ...
  bind_rows(uf_subd(ge_2024,q3_1,"2024","1")) %>%
  bind_rows(uf_subd(ge_2024,q3_2,"2024","2")) %>%
  bind_rows(uf_subd(ge_2024,q3_3,"2024","3")) %>%
  bind_rows(uf_subd(ge_2024,q3_5,"2024","4")) %>%
  bind_rows(uf_subd(ge_2024,q3_6,"2024","5")) %>%
  bind_rows(uf_subd(ge_2024,q3_7,"2024","6")) %>%
  bind_rows(uf_subd(ge_2024,q3_9,"2024","7")) %>%
  bind_rows(uf_subd(ge_2024,q3_8,"2024","8")) %>%
  bind_rows(uf_subd(ge_2024,q3_4,"2024","9")) %>%
...
No que se refere aos **municípios**, o @fig-munic_sub mostra **crescimento da constituição formal de todas as áreas administrativas**. As áreas do SUAS com maior percentual de constituição formal nos órgãos gestores municipais são, em ordem decrescente: Gestão do Cadastro Único e Programa Bolsa Família, Proteção Social Básica e Gestão do SUAS.

Já as áreas que possuem os menores percentuais de constituição formal são as de Gestão do Trabalho, de Vigilância Socioassistencial, de Regulação do SUAS e de Proteção Social Especial. A constituição da área de Proteção Social Especial é recomendada independente da presença de CREAS, e vem crescendo ano a ano, chegando em 2024 a 58,1% dos municípios com constituição formal desta área.

```{r fig-munic_sub, fig.cap = "Distribuição dos órgãos gestores municipais segundo constituição/formalização de subdivisões administrativas - Brasil; 2012, 2018, 2023 e 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
#| fig-height: 6
...
#  bind_rows(munic_sub(gm_2022,q1_4,"2022","9")) %>%
  ...
  bind_rows(munic_sub(gm_2024,q3_1,"2024","1")) %>%
  bind_rows(munic_sub(gm_2024,q3_2,"2024","2")) %>%
  bind_rows(munic_sub(gm_2024,q3_3,"2024","3")) %>%
  bind_rows(munic_sub(gm_2024,q3_5,"2024","4")) %>%
  bind_rows(munic_sub(gm_2024,q3_6,"2024","5")) %>%
  bind_rows(munic_sub(gm_2024,q3_7,"2024","6")) %>%
  bind_rows(munic_sub(gm_2024,q3_9,"2024","7")) %>%
  bind_rows(munic_sub(gm_2024,q3_8,"2024","8")) %>%
  bind_rows(munic_sub(gm_2024,q3_4,"2024","9")) %>%
...
A uniformização da Lei do SUAS dos estados e municípios em consonância com a LOAS foi deliberada na X Conferência Nacional, em dezembro de 2015. No Censo SUAS 2023, 6 estados haviam informado que a Lei Estadual de regulamentação do SUAS havia sido atualizada em 2023. No Censo SUAS 2024, mais 2 estados informaram ter **atualizado a Lei Estadual de regulamentação** em 2023, incluindo o **Piauí**, que até então não tinha Lei Estadual de regulamentação, chegando a 8 estados que informaram ter atualizado a Lei Estadual em 2023. Nenhum estado informou ter atualizado a Lei em 2024.

Como pode ser visto no @fig-estados-atualizacao-lei, um total de 14 estados informaram ter ocorrido atualização da Lei Estadual entre 2011 e 2022, e ainda restavam **4 estados que não possuíam Lei do SUAS**, sendo eles: **Roraima, Tocantins, São Paulo e Santa Catarina**.

```{r fig-estados-atualizacao-lei, fig.cap = "Quantidade de estados segundo último ano de atualização da Lei Estadual de regulamentação do Sistema Único de Assistência Social (SUAS) - Brasil, 2024", fig.footer = "Fonte: MDS, Censo SUAS."}

grafico<-ge_2024 %>%
  select(x = q4) %>%
  mutate(x = case_when(
   is.na(x) ~"Não possui lei", 
   x <= 2015~"De 2011 a 2015",
   x <= 2019~"De 2016 a 2019",
   x <= 2022~"De 2020 a 2022",
   x <= 2023~"Em 2023",
   x <= 2024~"Em 2024")) %>%
  mutate(x = factor(x, levels = c("Não possui lei", "De 2011 a 2015", "De 2016 a 2019", "De 2020 a 2022", "Em 2023", "Em 2024"))) %>%
...
Em 2024, **5,2%** dos 5.262 **municípios** que responderam ao Censo SUAS informaram ter ocorrido naquele mesmo ano **aprovação ou atualização da Lei Municipal de regulamentação do SUAS** e **19,7%** informaram ainda **não possuir lei** (@fig-municipios-atualizacao-lei).

```{r fig-municipios-atualizacao-lei, fig.cap = "Percentual de municípios segundo último ano de aprovação e/ou atualização da Lei Municipal de regulamentação do Sistema Único de Assistência Social (SUAS) - Brasil, 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
...
grafico <- gm_2024 %>%
  ...
  mutate(Percentual = n/sum(n)) %>%
  filter(x!="Até 2005") %>%
  mutate(x = factor(x, levels = c("Não possui lei", "De 2006 a 2010", "De 2011 a 2015", "De 2016 a 2019", "De 2020 a 2022", "Em 2023", "Em 2024"))) %>%
...
#  bind_rows(f_sim(ge_2015,q9,"2015")) %>%
  ...
  #2023 e 2024 não tem essa pergunta
...
Em 2024, manteve-se em **15**, pelo terceiro ano seguido, o número de **estados que possuíam Plano de Apoio Técnico aos municípios pactuado na Comissão Intergestores Bipartite (CIB)** (@fig-plan_apoio_tec). A realização deste plano pelos estados está prevista no último pacto de aprimoramento Estadual do SUAS ([Resolução CNAS Nº2, de 16 de março de 2017](https://blog.mds.gov.br/redesuas/resolucao-no-2-de-16-de-marco-de-2017/)).

```{r fig-plan_apoio_tec, fig.cap = "Quantidade de estados que possuem Plano de Apoio Técnico pactuado na CIB - Brasil, 2013 a 2024", fig.footer = "Fonte: MDS, Censo SUAS"}
...
grafico <- f_sim(ge_2013, q85, "2013") %>%
   ...
   bind_rows(f_sim(ge_2024, q26, "2024")) %>%
...
A NOB SUAS 2012 estabelece que os Planos de Assistência Social devem observar **ações de apoio técnico e financeiro à gestão descentralizada do SUAS** (Capítulo III - NOBSUAS/2012), compreendendo, entre outras ações:
...
Enquanto o Censo SUAS 2023 mostrou um aumento no número de estados que ofertaram cada tipo de ação de apoio técnico aos municípios, em todos os tipos de ações perguntados no Censo, no Censo SUAS 2024 não houve aumento no número de estados que ofertavam cada tipo de ação de apoio técnico aos municípios, em nenhum dos tipos de ações (@fig-estados-formas-apoio). Em 2024 foi registrada ou a manutenção do aumento observado em 2023, ou uma redução no número de estados, e em dois tipos de ação uma redução até em relação ao observado em 2022: **videoconferências e transmissões ao vivo, que havia aumentado de 19 para 22 estados em 2023, caiu para 18 estados em 2024; e monitoramento com periodicidade mínima anual, que havia aumentado de 17 para 22 estados em 2023, caiu para 16 estados em 2024**.

**Tanto em 2023 quanto em 2024 todos os 26 estados relataram ter realizado apoio técnico individualizado a municípios específicos e encontros/eventos de apoio técnico presenciais**. O número de estados que relataram possuir centrais de relacionamento manteve-se em 6, após ter dobrado de 3 para 6 estados de 2022 para 2023, permanecendo a ação das listadas no Censo com menor número de estados realizando.

```{r fig-estados-formas-apoio, fig.cap = "Quantidade de estados segundo formas de apoio técnico aos municípios - Brasil, 2022, 2023 e 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
...
tabela <- ge_apoio_tecnico(ge_2022, q21_1, "2022", "1") %>%
  ...
  bind_rows(ge_apoio_tecnico(ge_2024, q22_1, "2024", "1")) %>%
  bind_rows(ge_apoio_tecnico(ge_2024, q22_2, "2024", "2")) %>%
  bind_rows(ge_apoio_tecnico(ge_2024, q22_3, "2024", "3")) %>%
  bind_rows(ge_apoio_tecnico(ge_2024, q22_4, "2024", "4")) %>%
  bind_rows(ge_apoio_tecnico(ge_2024, q22_5, "2024", "5")) %>%
  bind_rows(ge_apoio_tecnico(ge_2024, q22_6, "2024", "6")) %>%
  bind_rows(ge_apoio_tecnico(ge_2024, q22_7, "2024", "7")) %>%
  bind_rows(ge_apoio_tecnico(ge_2024, q22_8, "2024", "8")) %>%
  bind_rows(ge_apoio_tecnico(ge_2024, q22_9, "2024", "9")) %>%
  bind_rows(ge_apoio_tecnico(ge_2024, q22_10, "2024", "10")) %>%
...
```{r fig-municipios-formas-apoio, fig.cap = "Percentual de municípios por formas de apoio técnico recebido do estado nos 12 meses anteriores à pesquisa, segundo grandes regiões - Brasil, 2024", fig.footer = "Fonte: MDS, Censo SUAS.", eval=FALSE}
...
# Questionários 2022:
  select(IBGE,
         "Participou de encontros de apoio técnico presenciais"=q5_1,
         "Recebeu apoio técnico individualizado do estado"=q5_2,
         "Enviou informações à Secretaria Estadual para monitoramento"=q5_3,
         "Participou de seminários"=q5_4,
         "Recebeu visitas técnicas do estado"=q5_5,
         "Entrou em contato com a central de relacionamento do estado"=q5_6,
         "Recebeu orientações técnicas e materiais informativos elaborados ou compartilhados pelo estado"=q5_7,
         "Participou de Videoconferências e transmissões ao vivo"=q5_8,
         "Não recebeu nenhum apoio técnico"=q5_0) %>%
...
grafico <- gm_visita_tecnicos(gm_2013, q19, "2013") %>%
  ...
  #2022, 2023 e 2024 muda a lógica da pergunta
...
Desde 2020 observa-se um aumento a cada ano no **percentual de municípios que informaram ter recebido visitas de apoio técnico do estado**, com aumento expressivo em 2024, **de 41,8% em 2023 para 58,9% dos municípios em 2024** (@fig-munic_vit_estadual).

```{r fig-munic_vit_estadual, fig.cap = "Percentual de municípios que informam receber visitas de apoio técnico do estado - Brasil, 2020 a 2024", fig.footer = "Fonte: MDS, Censo SUAS"}
...
grafico <- f_sim(gm_2020, q7_5, "2020") %>%
  ...
  bind_rows(f_sim(gm_2024, q5_3, "2024")) %>%
...
Observa-se em 2024 uma **redução de 24 para 21 no número de estados que possuíam Núcleo de Educação Permanente implantado de maneira formal**, menor número desde 2021, após aumento observado de 2020 a 2023, conforme mostrado no @fig-NUEP. Além do Estado de Rondônia, que era um dos dois que não possuíam em 2023 o Núcleo de Educação Permanente implantado de maneira formal, os estados da Paraíba, de Sergipe, do Mato Grosso do Sul e do Rio de Janeiro deixaram de ter em 2024, sendo que o Rio de Janeiro informou ter um Núcleo implantado de maneira informal. Já o Estado do Rio Grande do Sul, que era o outro que não possuía em 2023 o Núcleo de Educação Permanente implantado de maneira formal, informou possuir em 2024.

```{r fig-NUEP, fig.cap = "Quantidade de estados que possuem Núcleo de Educação Permanente implantado de maneira formal, 2020 a 2024", fig.footer = "Fonte: MDS, Censo SUAS"}
...
grafico <- nuep(ge_2020, q47, "2020") %>%
    ...
    bind_rows(nuep(ge_2024, q39, "2024")) %>%
...
Em relação ao **Plano de Educação Permanente**, houve pequeno aumento no Censo SUAS 2024 no percentual de municípios que possuíam este plano, de 18,6% para **19,0% dos municípios**, mantendo-se o mesmo patamar após aumento mais significativo ocorrido em 2023, sendo este o maior percentual da série histórica iniciada em 2015 (@fig-PNEP_Munic).

```{r fig-PNEP_Munic, fig.cap = "Percentual de municípios que possuem Plano de Capacitação e Educação Permanente, - Brasil; 2015 a 2024", fig.footer = "Fonte: MDS, Censo SUAS"}
...
grafico <- f_sim(gm_2015, q40, "2015") %>%
    ...
    bind_rows(f_sim(gm_2024, q69, "2024")) %>%
...
```{r fig-municipios-possuem-plano, fig.cap = "Percentual de municípios que possuem Plano de Capacitação e Educação Permanente, segundo grandes regiões - Brasil; 2015, 2017, 2023 e 2024", fig.footer = "Fonte: MDS, Censo SUAS",eval=FALSE}
...
grafico <- f_selecao_regiao_grupo(gm_2015, q40, "Sim", Região, "2015") %>%
  ...
  # bind_rows(f_selecao_regiao_ano(gm_2020, q73, "Sim", Região, "2020")) %>%
   ...
   bind_rows(f_selecao_regiao_grupo(gm_2023, q62, "Sim", IBGE, "2023")) %>%
    bind_rows(f_selecao_regiao_grupo(gm_2024, q69, "Sim", IBGE, "2024")) %>%
...
O modelo de gestão do SUAS prevê cofinanciamento compartilhado entre os três entes federados ([art. 30-A da Lei Orgânica da Assistência Social](https://www.planalto.gov.br/ccivil_03/leis/l8742.htm#art30a)). No Censo SUAS 2024, **pela primeira vez** desde o incício da série histórica em 2012, **todos os estados informaram ter cofinanciado os municípios**, como mostra o @fig-estados-cofinanciamento-municipios.

O cofinanciamento deve ser realizado por meio de transferência automática regular entre os fundos de Assistência Social (fundo-a-fundo). Em 2024, além de todos os estados terem cofinanciado os municípios, **todos realizaram cofinanciamento por meio de transferência fundo-a-fundo**.

```{r fig-estados-cofinanciamento-municipios, fig.cap = "Percentual de estados segundo realização de cofinanciamento aos municípios - Brasil; 2012 a 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
...
grafico <- ge_cofinanciamento(ge_2012, q64_0, "2012") %>%
  ...
  bind_rows(ge_cofinanciamento(feas_2023, q6, "2023")) %>%
   bind_rows(ge_cofinanciamento(feas_2024, q6, "2024")) %>%
...
Olhando para os blocos de financiamento nos quais ocorreram destinação dos recursos transferidos pelos estados aos municípios (@fig-estados-blocos-recursos), destaca-se no Censo SUAS 2024 o **aumento no número de estados que destinaram recursos ao bloco de incentivo financeiro para gestão do SUAS, de apenas 3 estados em 2023 para 9 estados em 2024**, mas este ainda permanece o bloco com menor número de estados que destinam recursos a ele.

```{r fig-estados-blocos-recursos, fig.cap = "Número de estados segundo a destinação dos recursos transferidos aos municípios por blocos de financiamento - Brasil; 2012, 2017, 2022, 2023 e 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
...
#  bind_rows(ge_destinacao_recursos(ge_2016, q34_5, "2016", "5")) %>%
  ...
  bind_rows(ge_destinacao_recursos(feas_2024, q10_1, "2024", "1")) %>%
  bind_rows(ge_destinacao_recursos(feas_2024, q10_2, "2024", "2")) %>%
  bind_rows(ge_destinacao_recursos(feas_2024, q10_3, "2024", "3")) %>%
  bind_rows(ge_destinacao_recursos(feas_2024, q10_4, "2024", "4")) %>%
  bind_rows(ge_destinacao_recursos(feas_2024, q10_5, "2024", "5")) 
...
A pessoa que exerce a função de ordenador de despesa responde pela autorização de pagamento dos recursos do Fundo de Assistência Social. Recomenda-se que ela tenha amplo conhecimento sobre a política. O @fig-estado_ord_despesa mostra que, historicamente, **entre 73,1% e 88,5% dos estados têm a/o secretária/o estadual de assistência social como a pessoa responsável pela ordenação de despesa, que é a pessoa ideal para essa função**. Em 2024 esse percentual foi de 80,8%, uma queda em relação a 2023.

Assim como em 2023, em 2024 não foi registrado em nenhum estado que esta função tenha sido exercida por secretária/o ou técnica/o de outra área. Nos 19,2% dos estados em que a função não foi exercida pela/o secretária/o estadual de assistência social, ela foi exercida por outra/o funcionária/o da secretaria de assistência social. Apenas em 2012 foi registrado 1 estado em que a pessoa ordenadora de despesa foi o/a governador/a do estado.

```{r fig-estado_ord_despesa, fig.cap = "Percentual de estados quanto a ordenação de despesa do Fundo Estadual - Brasil, 2012 a 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
...
grafico <- ord_despesa(ge_2012, q44, "2012") %>%
  ...
  bind_rows(ord_despesa(feas_2024, q2, "2024")) %>% 
...
No que se refere aos municípios, **aumentou de 81,5% em 2023 para 82,1% em 2024 o percentual dos municípios que têm a/o secretária/o municipal de assistência social como a pessoa ordenadora de despesa do Fundo Municipal de Assistência Social**. Esse percentual vem aumentando desde o começo desta série histórica em 2012, quando era de apenas 52,9%. Ao mesmo tempo observa-se mais uma **redução no percentual de municípios que têm a/o prefeita/o como a pessoa ordenadora de despesa da política de assistência social, caindo de 15,2% dos municípios em 2023 para 14,7% em 2024**, percentual que era 39,6% no início desta série histórica em 2012 (@fig-munic_ord_despesa).

É considerado positivo esse movimento constante ao longo do tempo de aumento no percentual de municípios que têm como pessoa ordenadora de despesa do Fundo Municipal de Assistência Social a/o secretária/o municipal de assistência social, e redução no percentual dos municípios que têm a/o prefeita/o exercendo esta função, pois o gestor municipal, por estar mais próximo do cotidiano do planejamento desta política pública, provavelmente está em melhores condições para tomar as decisões sobre o orçamento.

```{r fig-munic_ord_despesa, fig.cap = "Percentual de municípios quanto a ordenação de despesa do Fundo Municipal de Assistência Social - Brasil, 2012 a 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
...
grafico <- ord_despesa(gm_2012, q28, "2012") %>%
  ...
  bind_rows(ord_despesa(fmas_2024, q2, "2024")) %>%
...
Em relação aos **programas próprios de transferência de renda executados pelos estados, registrou-se em 2024 o maior número de toda série histórica**, desde 2012, de estados que informaram possuir programas próprios, chegando a **20 estados** (@fig-be_uf_renda).
...
```{r fig-be_uf_renda, fig.cap = "Quantidade de estados que possuem programa próprio de transferência de renda - Brasil, 2012 a 2024", fig.footer = "Fonte: MDS, Censo SUAS"}
...
grafico <- f_sim(ge_2012, q33, "2012") %>%
  ...
  bind_rows(f_sim(ge_2024, q7, "2024")) %>%
...
O Cadastro Único para Programas Sociais do Governo Federal (Cadastro Único), instituído pelo [art. 6º-F da Lei Orgânica da Assistência Social](https://www.planalto.gov.br/ccivil_03/leis/l8742.htm#art6f) e regulamentado atualmente pelo [Decreto nº 11.016, de 29 de março de 2022](https://www.planalto.gov.br/ccivil_03/_ato2019-2022/2022/decreto/d11016.htm), tem a finalidade de coletar, sistematizar e disseminar informações que permitam a identificação e caracterização das condições socioeconômicas das famílias em situação de vulnerabilidade social, sobretudo para as famílias de baixa renda.
...
As unidades do Cadastro Único podem ser encontradas em locais exclusivos, ou na rede de atendimento socioassistencial de CRAS, CREAS e Centro POP. Em 2020 foi introduzido no Censo SUAS um formulário sobre os postos exclusivos do Cadastro Único.

Apesar da maioria dos locais de cadastramento serem os CRAS, com quase o dobro de unidades em relação ao número de postos exclusivos para o Cadastro Único, **o número destes postos exclusivos vem crescendo a cada ano, enquanto o número de CRAS que realizam cadastramento ou atualização do Cadastro Único foi menor em 2024 do que nos dois anos anteriores** (@fig-unidades-cadunico).
...
```{r fig-unidades-cadunico, fig.cap = "Quantidade de Postos de Cadastro Único e de outras unidades que realizam cadastramento ou atualização do Cadastro Único - Brasil, 2018 a 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
#| fig-height: 6

# Gráfico criado para substituir a tabela acima
# Verificar alguma forma melhor de apresentação, que não coloque o total e as parcelas no mesmo eixo e mesma escala. Por exemplo, fazer um gráfico separado para o total, ou fazer um gráfico com as parcelas empilhadas, mostrando na mesma barra os números de cada parcela e o número total.

qtd_unidades_cadunico = function(df, q, ano, cod_categoria){
  q <- enquo(q)
  ano <- quo_name(ano)
  cod_categoria <- quo_name(cod_categoria)

  categoria <- case_when(!! cod_categoria == "1"~"CRAS",
                         !! cod_categoria == "2"~"CREAS",
                         !! cod_categoria == "3"~"Centro POP")

  df %>%
    select(!! q) %>%
    filter(!! q == "Sim, com equipe exclusiva para esta finalidade" |
           !! q == "Sim, com a equipe responsável pelo PAIF" |
           !! q == "Sim, com a equipe responsável pelo PAEFI" |
           !! q == "Sim, com a equipe responsável pelo Serviço Especializado para Pessoas em Situação de Rua" |
           !! q == "Sim, com outra equipe" |
# Em 2020 o arquivo de dados está com códigos numéricos em vez do texto das respostas, e os códigos 1, 2 e 3 correspondem às respostas "Sim"
           !! q == "1" | 
           !! q == "2" |
           !! q == "3") %>%
    f_quantitativo_em_2_grupos(categoria, ano)
}

postos <- "Postos de Cadastro Único"

# Em 2017 a pergunta era diferente, por isso não foi incluído no gráfico.
# Verificar se, para uniformidade da série histórica, não é melhor fazer a série apenas a partir de 2020, quando passou a ter os formulários para os postos de cadastro único, .

tabela <- 
  qtd_unidades_cadunico(cras_2018, q_40, "2018", "1") %>%
  bind_rows(qtd_unidades_cadunico(creas_2018, q44, "2018", "2")) %>%
  bind_rows(qtd_unidades_cadunico(cpop_2018, q27, "2018", "3")) %>%

  bind_rows(qtd_unidades_cadunico(cras_2019, q39, "2019", "1")) %>%
  bind_rows(qtd_unidades_cadunico(creas_2019, q42, "2019", "2")) %>%
  bind_rows(qtd_unidades_cadunico(cpop_2019, q26, "2019", "3")) %>%

  bind_rows(qtd_unidades_cadunico(cras_2020, q39, "2020", "1")) %>%
  bind_rows(qtd_unidades_cadunico(creas_2020, q47, "2020", "2")) %>%
  bind_rows(qtd_unidades_cadunico(cpop_2020, q35, "2020", "3")) %>%
  bind_rows(f_quantitativo_em_2_grupos(posto_cadunico_2020, postos, "2020")) %>%

  bind_rows(qtd_unidades_cadunico(cras_2021, q39, "2021", "1")) %>%
  bind_rows(qtd_unidades_cadunico(creas_2021, q47, "2021", "2")) %>%
  bind_rows(qtd_unidades_cadunico(cpop_2021, q35, "2021", "3")) %>%
  bind_rows(f_quantitativo_em_2_grupos(posto_cadunico_2021, postos, "2021")) %>%

  bind_rows(qtd_unidades_cadunico(cras_2022, q39, "2022", "1")) %>%
  bind_rows(qtd_unidades_cadunico(creas_2022, q47, "2022", "2")) %>%
  bind_rows(qtd_unidades_cadunico(cpop_2022, q35, "2022", "3")) %>%
  bind_rows(f_quantitativo_em_2_grupos(posto_cadunico_2022, postos, "2022")) %>%

  bind_rows(qtd_unidades_cadunico(cras_2023, q35, "2023", "1")) %>%
  bind_rows(qtd_unidades_cadunico(creas_2023, q39, "2023", "2")) %>%
  bind_rows(qtd_unidades_cadunico(cpop_2023, q28, "2023", "3")) %>%
  bind_rows(f_quantitativo_em_2_grupos(posto_cadunico_2023, postos, "2023")) %>%

  bind_rows(qtd_unidades_cadunico(cras_2024, q49, "2024", "1")) %>%
  bind_rows(qtd_unidades_cadunico(creas_2024, q40, "2024", "2")) %>%
  bind_rows(qtd_unidades_cadunico(cpop_2024, q28, "2024", "3")) %>%
  bind_rows(f_quantitativo_em_2_grupos(posto_cadunico_2024, postos, "2024")) %>%

  pivot_wider(names_from = Grupo1, values_from = n, values_fill = 0) %>%
  mutate("Total" = rowSums(.[2:5])) %>%
  pivot_longer(-Grupo2, names_to = "Grupo1", values_to = "n") %>%
  filter(n != 0)

ordenado<-tabela %>%
  filter(Grupo2 == ano_atual) %>%
  arrange(n) %>%
  pull(Grupo1)

grafico<-tabela %>%
  mutate(Grupo1 = factor(Grupo1, levels = rev(ordenado))) %>%
  f_grafico_col_numero_flip_2_x(Grupo1, n, Grupo2)

grafico
```

O @fig-local-posto-cadastramento mostra os **locais de funcionamento dos Postos de Cadastro Único**. Vemos que **a maioria funciona na Sede da Secretaria da Assistência Social ou congênere, mas este percentual vem caindo gradualmente a cada ano, chegando a 56,6% em 2024. Já o percentual de postos que funcionam em locais exclusivos para atendimento do Cadastro Único vem crescendo desde 2022, chegando a 27,5% em 2024**.

```{r fig-local-posto-cadastramento, fig.cap = "Percentual de Postos de Cadastro Único segundo local do Posto - Brasil, 2020 a 2024", fig.footer = "Fonte: MDS, Censo SUAS"}
#| fig-height: 7
...
tabela <-
  ...
  bind_rows(posto_local(posto_cadunico_2024, q1, "2024"))

ordenado<-tabela %>%
  filter(Categoria == ano_atual) %>%
  arrange(n) %>%
  pull(Nível)
...
No que se refere à **promoção pelos municípios de ações de capacitação sobre o Cadastro Único**, observamos no @fig-capac_cad uma inversão em 2024 na tendência observada de 2020 a 2023. O **percentual de municípios que não promoveram estas ações de capacitação**, que havia caído de 46,6% em 2020 para 29,3% em 2023, **subiu para 33,6% em 2024**. No sentido inverso, todos os meios de promoção de ações de capacitação que vinham crescendo a cada ano caíram em 2024 em relação a 2023. 

A promoçao de ações de capacitação por instituições de ensino, que havia crescido até 2022, teve mais uma pequena queda em 2024, ficando em 5,0% e continuando a ser o meio dentre os listados no questionário do Censo menos utilizado pelos municípios para promoção de ações de capacitação sobre o Cadastro Único.

```{r fig-capac_cad, fig.cap = "Percentual de municípios que promoveram ações de capacitação sobre o Cadastro Único segundo meios utilizados - Brasil, 2020 a 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
#| fig-height: 6
...
#2020
 ...
 tabela <-
  ...
  bind_rows(capac_cad(gm_2024,q65_1,"2024","1")) %>%
  bind_rows(capac_cad(gm_2024,q65_2,"2024","2")) %>%
  bind_rows(capac_cad(gm_2024,q65_3,"2024","3")) %>%
  bind_rows(capac_cad(gm_2024,q65_4,"2024","4")) %>%
  bind_rows(capac_cad(gm_2024,q65_99,"2024","99")) %>%
  bind_rows(capac_cad(gm_2024,q65_0,"2024","0")) %>%
...
**O percentual de municípios que realizavam e acompanhavam a inserção e atualização das pessoas beneficiárias do BPC no Cadastro Único teve mais uma queda em 2024, pelo terceiro ano seguido, ficando em 82,2% dos municípios**, como mostrado no @fig-BPC_cadunico.

```{r fig-BPC_cadunico, fig.cap = "Percentual de municípios cuja Assistência Social realiza e acompanha a inserção e atualização das/os beneficiárias/os do BPC no Cadastro Único - Brasil, 2020 a 2024", fig.footer = "Fonte: MDS, Censo SUAS"}
grafico <-
  ...
  bind_rows(f_sim(gm_2024, q66_6, "2024")) %>%
...
O @fig-ave_cad mostra o percental de Postos de Cadastro Único que fazem ações específicas para atender demandas de Averiguação e Revisão Cadastral. **A busca ativa das famílias de Averiguação e Revisão Cadastral é a ação mais realizada e chegou em 2024 ao percentual máximo da série histórica desde 2020, com 85,8% dos Postos de Cadastro Único** relatando a realização desta ação. A segunda ação mais realizada é a colocação das famílias de Averiguação e Revisão Cadastral no público prioritário de atendimento na unidade, que também chegou em 2024 ao maior percentual observado na série histórica desde 2020, com 70,3% dos Postos de Cadastro Único relatando a realização desta ação.

```{r fig-ave_cad, fig.cap = "Percentual de Postos de Cadastro Único que fazem ações específicas para atender demandas de Averiguação e Revisão Cadastral - Brasil, 2020 a 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
...
#2020
 ...
 tabela <- ave_cad (posto_cadunico_2020,q18_1,"2020","1") %>%
  ...
  bind_rows(ave_cad(posto_cadunico_2024,q13_1,"2024","1")) %>%
  bind_rows(ave_cad(posto_cadunico_2024,q13_2,"2024","2")) %>%
  bind_rows(ave_cad(posto_cadunico_2024,q13_3,"2024","3")) %>%
  bind_rows(ave_cad(posto_cadunico_2024,q13_99,"2024","4")) %>%
  bind_rows(ave_cad(posto_cadunico_2024,q13_0,"2024","5")) %>%
...
O @fig-visit_dom mostra as situações que levam os Postos de Cadastro Único a realizarem entrevistas domiciliares. Destaca-se nos últimos 2 anos da série, 2023 e 2024, **aumento expressivo**, em torno de 10 pontos percentuais em cada ano, **dos postos que relataram realizar entrevistas domiciliares para famílias incluídas em Averiguação Cadastral, partindo de 42,3% dos postos em 2022 e chegando em 62,3% dos postos em 2024**, situação que é a mais frequente motivação para realização das entrevistas domiciliares pelos Postos de Cadastro Único.

Destaca-se também em 2024 o aumento no percentual de postos que realizaram entrevistas domiciliares para inclusão ou atualização de dados de famílias com BPC, que vinha de uma sequencia de quedas em torno de 1 ponto percentual a cada ano, de 2020 a 2023, e subiu mais de 6 pontos percentuais em 2024, chegando a 46,3% dos postos. O percentual de postos que realizaram entrevistas domiciliares para a inclusão ou atualização cadastral também teve significativo aumento em 2024, de 37,7% em 2023 para 44,5%.

```{r fig-visit_dom, fig.cap = "Percentual de Postos de Cadastro Único por situações em que são realizadas entrevistas domiciliares - Brasil, 2020 a 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
#| fig-height: 6
...
#2020
 ...
 tabela <- visit_dom (posto_cadunico_2020,q19_1,"2020","1") %>%
  ...
  bind_rows(visit_dom(posto_cadunico_2024,q14_1,"2024","1")) %>%
  bind_rows(visit_dom(posto_cadunico_2024,q14_2,"2024","2")) %>%
  bind_rows(visit_dom(posto_cadunico_2024,q14_3,"2024","3")) %>%
  bind_rows(visit_dom(posto_cadunico_2024,q14_4,"2024","4")) %>%
  bind_rows(visit_dom(posto_cadunico_2024,q14_5,"2024","5")) %>%
  bind_rows(visit_dom(posto_cadunico_2024,q14_6,"2024","6")) %>%
  bind_rows(visit_dom(posto_cadunico_2024,q14_99,"2024","7")) %>%
...
Vemos no @fig-redesuas os percentuais dos diversos **tipos de relacionamento dos Postos de Cadastro Único com os outros equipamentos da rede socioassistencial** (CRAS, CREAS, Centro POP, entre outros). Observa-se pouca variação nos percentuais em cada tipo de relacionamento, sendo **os tipos mais presentes, em mais de 90% dos postos, o encaminhamento das famílias para a rede socioassistencial de referência para inclusão em serviços socioassistenciais e o atendimento de famílias para inclusão e atualização cadastral encaminhadas pela rede socioassistencial**. Menos de 1% dos postos relatam não se relacionar com outros equipamentos da rede socioassistencial.

```{r fig-redesuas, fig.cap = "Percentual de Postos de Cadastro Único segundo relação com os outros equipamentos da rede socioassistencial - Brasil, 2020 a 2024", fig.footer = "Fonte: MDS, Censo SUAS."}
#| fig-height: 6
...
#2022
 ...
 tabela <- rede_suas (posto_cadunico_2020,q32_1,"2020","1") %>%
  ...
  bind_rows(rede_suas(posto_cadunico_2024,q16_1,"2024","1")) %>%
  bind_rows(rede_suas(posto_cadunico_2024,q16_2,"2024","2")) %>%
  bind_rows(rede_suas(posto_cadunico_2024,q16_3,"2024","3")) %>%
  bind_rows(rede_suas(posto_cadunico_2024,q16_4,"2024","4")) %>%
  bind_rows(rede_suas(posto_cadunico_2024,q16_5,"2024","5")) %>%
  bind_rows(rede_suas(posto_cadunico_2024,q16_0,"2024","6")) %>%
